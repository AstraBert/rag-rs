...
const binp = path.join(cargoDir, "bin", "rag-rs");

if (fs.existsSync(binp)) {
  console.log("Uninstalling rag-rs...");
  exec(`cargo uninstall rag-rs`, (error, stdout, stderr) => {
    console.log(stdout);
    if (error || stderr) {
  ...
  console.log("rag-rs not found skipping!");
}
